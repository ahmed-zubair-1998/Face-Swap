...
trg = cv2.imread('target.jpg',1).astype(np.uint8)
idx = 0

for f in sorted(glob.glob('sources/*.jpg')):
  ...
  edges = np.array([[[0,0], [0,height-1], [width-1,height-1], [width-1, 0]]]).astype(np.float32)
  ...
  cv2.imwrite("results/" + str(idx) + ".jpg", output)
  #cv2.imwrite("origs/" + str(idx) + ".jpg", src_cpy)
  idx += 1
  print(idx)